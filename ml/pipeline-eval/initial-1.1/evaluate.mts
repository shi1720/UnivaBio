...
import {analyzeDocument as hybrid} from './snapshot/lib/engine.ts';
import {analyzeDocument as rulesOnly} from './snapshot/lib/engine.rules-only.ts';
...
assert.equal(manifest.dataset_sha256,lock.dataset_sha256);
for(const [name,digest] of Object.entries(manifest.files))assert.equal(sha(readFileSync(new URL('./snapshot/'+name,import.meta.url))),digest,'Frozen engine source was modified: '+name);
assert.equal(sha(readFileSync(new URL('./snapshot/lib/engine.rules-only.ts',import.meta.url))),manifest.rules_only_adapter_sha256);
...
const results:any={schema_version:1,dataset_sha256:lock.dataset_sha256,engine_manifest:manifest,
...
writeFileSync(new URL('./errors.json',import.meta.url),JSON.stringify(Object.fromEntries(Object.entries(results.modes).map(([mode,value]:[string,any])=>[mode,value.documents.flatMap((doc:any)=>doc.errors.map((error:any)=>({document_id:doc.document_id,...error})))])),null,2)+'\n');
console.log(JSON.stringify({dataset_sha256:lock.dataset_sha256,hybrid:results.modes.hybrid.summary,rules_only:results.modes.rules_only.summary,documents_changed_by_ml:results.mode_differences.length,network_attempts:networkAttempts,boundary_probes:results.modes.hybrid.boundary_probes},null,2));
