...
The evaluator verifies frozen hashes before running. It writes `results.json` and `errors.json`; the report writer reads those files without inference. To evaluate a later engine, use `prepare_release.py` to create a new folder labeled as a release regression instead of overwriting this snapshot.
