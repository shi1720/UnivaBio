...
**Evaluation phase:** `release_regression_on_seen_fixtures`.
...
**This run is a release regression on already-seen fixtures.** It measures repairs against known examples. It must not be described as unseen evaluation or evidence of generalization.

## Current results in this result file

| Metric | Result |
|---|---:|
| Correct typed suggestions | 33 / 34 suggestions |
| Gold actions found with correct type | 33 / 44 |
| Suggestion precision | 97.1% |
| Suggestion recall | 75.0% |
| Suggestion F1 | 0.846 |
| Strict false positives / false negatives | 1 / 11 |
...
| Exact source spans, suggested cards | 34 / 34 |
...
| Conditional actions correctly typed or explicitly sent for review | 4 / 4 |
| Documents with perfect action precision and recall | 12 / 20 |
...
| Mixed / negation / history / conditions | 11 | 8 | 8 | 100.0% | 72.7% |
...
Hybrid: precision 97.1%, recall 75.0%. Rules only: precision 97.1%, recall 75.0%.
...
```sh
./node_modules/.bin/tsx /absolute/path/to/univabio-pipeline-eval/evaluate.mts
python3 /absolute/path/to/univabio-pipeline-eval/write_report.py
```
...
Initial/current engine SHA-256: `b4f8bebb0a0af41b4a3286bff67caaa580a748197a51ead2aac4083838b31dcc`.
