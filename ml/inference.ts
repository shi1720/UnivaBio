...
function validateModel(model: SentenceModel): void {
  if (
    ...
    model.labels.length !== CATEGORIES.length ||
    ...
    model.weights.some(
      (row) =>
        row.length !== model.vocabulary.length ||
...
export function createSentenceClassifier(model: SentenceModel) {
  ...
  return function classifySentence(text: string): SentencePrediction {
    ...
    const truncated =
      characters.length > h.max_chars || allTokens.length > h.max_tokens;
    ...
    const norm =
      Math.sqrt([...features.values()].reduce((sum, n) => sum + n * n, 0)) || 1;
    ...
    const logits = model.weights.map(
      (weights, k) =>
        model.intercepts[k] +
        x.reduce((sum, [j, value]) => sum + weights[j] * value, 0),
    );
    ...
    const coverage =
      tokens.filter((token) => vocab.has(`u:${token}`)).length /
      Math.max(1, tokens.length);
    const reason: AbstentionReason | null = truncated
      ? "input_too_long"
      : tokens.length < h.min_tokens
        ? "too_few_tokens"
        : coverage < h.min_coverage
          ? "low_vocabulary_coverage"
          : best.score < h.min_score
            ? "low_model_score"
            : margin < h.min_margin
              ? "ambiguous_scores"
              : null;
    return {
      ...
      scores: Object.fromEntries(
        CATEGORIES.map((label, i) => [label, probabilities[i]]),
      ) as Record<SentenceCategory, number>,
      reason,
    };
  };
}
